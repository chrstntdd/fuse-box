    ...
    "Should allow for variables to be aliased"() {
        return FuseTestEnv.create({
            project: {
                ...
                files: {
                    "index.ts": `import './main.css'`,
                    "main.css": `.test { background-image: url($imageRoot + '/something/else'); }`,
                }
            }
        }).simple().then((env) => env.browser((window) => {
            should(window.document.querySelectorAll('style')).haveLength(1);
            should(window.document.querySelector('style').innerHTML).equal(`.test { background-image: url(/ + '/something/else'); }`);
        }));
    }
}
